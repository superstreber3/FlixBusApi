...
app.post('/createStation', function (req, res) {
    ...
    db.run(insertQuery, [req.body.name, req.body.longitude, req.body.latitude, req.body.address, req.body.country, req.body.zip, req.body.city], function (err) {
        if (err) {
            console.log(err);
...
app.post('/createTour', function (req, res) {
    ...
    db.run(insertCustomerQuery, [req.body.firstName, req.body.lastName, req.body.email], function (err) {
        if (err) {
            console.log(err);
            res.status(500).send(err);
        } else {
            ...
            db.run(insertTourQuery, [req.body.startStationId, req.body.endStationId, customerId], function (err) {
                if (err) {
                    console.log(err);
                    res.status(500).send(err);
                } else {
